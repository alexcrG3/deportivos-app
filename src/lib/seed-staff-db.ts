...
export async function ensureStaffDBDataSeeded() {
  ...
  try {
    ...
    if (activeCoaches.length > 0) {
      ...
      const { data: dbAsistencia } = await supabase.from("asistencias_staff").select("id").eq("organizacion_id", orgId);
      if (!dbAsistencia || dbAsistencia.length === 0) {
        ...
        await supabase.from("asistencias_staff").upsert(asistenciasSeed);
        console.log("[Staff DB Seed] ✅ Asistencias registradas en Supabase BD");
      }

      // 3. Ensure "solicitudes_permisos" table is populated
      const { data: dbSolicitudes } = await supabase.from("solicitudes_permisos").select("id").eq("organizacion_id", orgId);
      if (!dbSolicitudes || dbSolicitudes.length === 0) {
        ...
        await supabase.from("solicitudes_permisos").upsert(solicitudesSeed);
        console.log("[Staff DB Seed] ✅ Solicitudes de permiso registradas en Supabase BD");
      }

      // 4. Ensure "certificaciones_staff" table is populated
      const { data: dbCerts } = await supabase.from("certificaciones_staff").select("id").eq("organizacion_id", orgId);
      if (!dbCerts || dbCerts.length === 0) {
        ...
        await supabase.from("certificaciones_staff").upsert(certsSeed);
        console.log("[Staff DB Seed] ✅ Certificaciones registradas en Supabase BD");
      }

      // 5. Ensure "evaluaciones_staff" table is populated
      const { data: dbEvals } = await supabase.from("evaluaciones_staff").select("id").eq("organizacion_id", orgId);
      if (!dbEvals || dbEvals.length === 0) {
        ...
        await supabase.from("evaluaciones_staff").upsert(evalsSeed);
        console.log("[Staff DB Seed] ✅ Evaluaciones del staff registradas en Supabase BD");
      }

      // 6. Ensure "nominas_entrenadores" table is populated with exact payroll records
      const { data: dbNominas } = await supabase.from("nominas_entrenadores").select("id").eq("organizacion_id", orgId);
      if (!dbNominas || dbNominas.length === 0) {
        ...
        await supabase.from("nominas_entrenadores").upsert(nominasSeed);
        console.log("[Staff DB Seed] ✅ Registros de Nómina guardados en Supabase BD");
      }
    }
  } catch (err) {
