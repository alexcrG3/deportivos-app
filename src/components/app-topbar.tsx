...
export function AppTopbar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-2 border-b bg-background/80 px-3 backdrop-blur-md lg:px-6 w-full max-w-full overflow-hidden">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="gap-1.5 px-2 text-xs">
              <span className="hidden sm:inline text-muted-foreground">Sede:</span>
              <span className="font-medium truncate max-w-[110px] sm:max-w-[150px]">{sede}</span>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <div className="relative hidden xl:block w-56">
          ...
          <Input placeholder="Buscar..." className="pl-9 h-9 bg-muted/40 border-transparent focus-visible:bg-background text-xs" />
        </div>
        ...
        <Button 
          ...
          className="hidden xl:flex gap-1.5 font-extrabold text-xs bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 rounded-xl shadow-sm shrink-0"
        >
          <Rocket className="h-4 w-4 text-amber-500 animate-pulse" />
          <span>Ruta NexusSport</span>
        </Button>
        ...
        <Button 
          ...
          className="hidden xl:flex gap-1.5 font-bold text-xs bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 rounded-xl shrink-0"
        >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="gap-2 px-1.5 h-9 shrink-0">
              <Avatar className="h-7 w-7 sm:h-8 sm:w-8 shrink-0">
                <AvatarFallback className="bg-gradient-primary text-primary-foreground text-xs font-semibold">
                  {initials}
                </AvatarFallback>
              </Avatar>
              <div className="hidden lg:flex flex-col items-start leading-tight max-w-[120px]">
                <span className="text-xs font-semibold truncate w-full text-left">{displayName}</span>
                <span className="text-[10px] text-muted-foreground truncate w-full text-left">{displayRole}</span>
              </div>
            </Button>
          </DropdownMenuTrigger>
